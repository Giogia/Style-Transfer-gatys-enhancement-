...
def conv_layer(image, filter_number, filter_size, strides, is_relu=True):
    ...
    # YOU MAKE THE CONVOLUTION OF THE IMAGE ND YOU RETURN THE CONVOLUTION

    weights_initialization = conv_initialization_vars(image, filter_number, filter_size)

    strides_shape = [1, strides, strides, 1]

    # we apply the filter to the image with a 2d convolution
...
def _instance_norm(img, train=True):
    ...
    # here we set the shape of the input img

    batch_size, rows, cols, in_channels = [i for i in img.get_shape()]

    var_shape = [in_channels]

    # we calculate the mean and the variance of the img
    ...
    # we normalize the img input wrt the mean and the variance calculated
